...
try:
    ...
    # Quotes are more likely to hit Yahoo's rate limit without it.
    _YF_SESSION = None
...
def get_quote(ticker, retries=3):
    for attempt in range(retries):
        try:
            ...
            wait = 3 * (attempt + 1)
            print(f"[warn] quote attempt {attempt + 1}/{retries} failed for {ticker}: {ex} — retrying in {wait}s")
            time.sleep(wait)
    print(f"[warn] quote failed for {ticker} after {retries} attempts")
    return None
